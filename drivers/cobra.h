...
class Cobra {
public:
    Cobra(const float &vref, std::shared_ptr<VMXPi> vmx = std::make_shared<VMXPi>(true, 50));
    ~Cobra();

    float GetVoltage(uint8_t ch);
    int GetRawValue(uint8_t ch);
