...
public:
    CobraSensor(std::shared_ptr<VMXPi> vmx, const std::string &name, const float &vRef, const int &mux_ch);
...
private:
    ...
    void Spin();
    int GetSingle(uint8_t channel);  // communicate with the ADC and get a single-ended input
    void Delay(double seconds);
    bool IsConnected(); // verify i2c connection of cobra sensor
