...
class Titan : public Device {
public:
    // constructor, destructor
    Titan(std::shared_ptr<VMXPi> vmx, const std::string &name, const uint8_t &canID, const uint16_t &motorFreq, const uint8_t &nEncoder, const float &distPerTick, const float &speed);
    // ~Titan(); //      Terminate titan1 -> (Re)start titan2 ==> SEG FAULT
...
private:
    // ROS
    void publish_titan_data();
    void Spin();

    std::shared_ptr<VMXPi> vmx_;
            // titan vmx def: VMXPi vmx{true, (uint8_t) 50};
    rclcpp::Publisher<std_msgs::msg::String>::SharedPtr titan_publisher_;
