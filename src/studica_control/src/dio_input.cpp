...
int main(int argc, char *argv[])
{
    ...
    try
    {
        if (vmx.IsOpen())
        {
            ...
            /* Note:  4 of the CommDIO Channels are not input capable */
            uint8_t num_input_capable_comm_dio_channels = num_comm_dio_channels - 4;
            ...
            VMXErrorCode vmxerr;
            VMXResourceHandle digitalio_res_handles[num_flexio_channels + num_hicurrdio_input_channels + num_input_capable_comm_dio_channels];
            ...
            for (int dio_channel_index = first_flexio_channel_index; dio_channel_index < first_flexio_channel_index + num_flexio_channels; dio_channel_index++)
            {
                ...
                if (!vmx.io.ActivateSinglechannelResource(VMXChannelInfo(dio_channel_index, VMXChannelCapability::DigitalInput), &dio_config,
                                                          digitalio_res_handles[digital_input_res_handle_index], &vmxerr))
                {
            ...
            /* Configure all input-capable CommDIOs as INPUTS. */
            ...
            {
                if (vmx.io.ChannelSupportsCapability(dio_channel_index, VMXChannelCapability::DigitalInput))
                {
                    DIOConfig dio_config;
                    if (!vmx.io.ActivateSinglechannelResource(VMXChannelInfo(dio_channel_index, VMXChannelCapability::DigitalInput), &dio_config,
                                                              digitalio_res_handles[digital_input_res_handle_index], &vmxerr))
