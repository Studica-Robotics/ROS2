...
void DiffDrive::publish_odometry() {
    int32_t left_count = titan_->GetEncoderCount(left_);
    int32_t right_count = titan_->GetEncoderCount(right_);
    ...
    if (log_counter++ % 20 == 0) {  // Log every 20 calls (~1 second at 50ms intervals)
        RCLCPP_INFO(this->get_logger(), 
            "Encoder counts - Left: %d (%.3fm), Right: %d (%.3fm)", 
            left_count, left_encoder, right_count, -right_encoder);
    }
