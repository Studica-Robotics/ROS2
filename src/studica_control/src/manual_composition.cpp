...
public:
    ...
    bool initialize() {
		...
		bool diff_drive_component_enabled = this->get_parameter("diff_drive_component.enabled").as_bool();
		...
		bool mecanum_drive_component_enabled = this->get_parameter("mecanum_drive_component.enabled").as_bool();
        ...
        if (diff_drive_component_enabled && mecanum_drive_component_enabled) {
		...
		if (diff_drive_component_enabled) {
            auto odom = studica_control::DiffOdometry::initialize(this);
			auto diff_drive_component_node = studica_control::DiffDrive::initialize(this, odom, vmx_);
            component_nodes.push_back(odom);
            component_nodes.push_back(diff_drive_component_node);
		}
		...
		if (mecanum_drive_component_enabled) {
            auto odom = studica_control::MecanumOdometry::initialize(this);
			auto mecanum_drive_component_node = studica_control::MecanumDrive::initialize(this, odom, vmx_);
            component_nodes.push_back(odom);
            component_nodes.push_back(mecanum_drive_component_node);
		}
...
int main(int argc, char *argv[]) {
    ...
    if (control_server->initialize()) control_server->run();
