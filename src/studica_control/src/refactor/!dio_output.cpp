...
int main(int argc, char *argv[])
{
    ...
    try
    {
        if (vmx.IsOpen())
        {
            ...
            /* Note:  2 of the CommDIO Channels are not output capable */
            uint8_t num_output_capable_comm_dio_channels = num_comm_dio_channels - 2;
            ...
            VMXErrorCode vmxerr;
            VMXResourceHandle digitalio_res_handles[num_flexio_channels + num_hicurrdio_output_channels + num_output_capable_comm_dio_channels];
            ...
            for (int dio_channel_index = first_flexio_channel_index; dio_channel_index < first_flexio_channel_index + num_flexio_channels; dio_channel_index++)
            {
                ...
                if (!vmx.io.ActivateSinglechannelResource(VMXChannelInfo(dio_channel_index, VMXChannelCapability::DigitalOutput), &dio_config,
            ...
            /* Configure all output-capable CommDIOs as OUTPUTS. */
            ...
            {
                if (vmx.io.ChannelSupportsCapability(dio_channel_index, VMXChannelCapability::DigitalOutput))
                {
                    DIOConfig dio_config(DIOConfig::OutputMode::PUSHPULL);
                    if (!vmx.io.ActivateSinglechannelResource(VMXChannelInfo(dio_channel_index, VMXChannelCapability::DigitalOutput), &dio_config,
                                                              digitalio_res_handles[digital_output_res_handle_index], &vmxerr))
